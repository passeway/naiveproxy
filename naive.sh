...
if ! chmod +x /usr/bin/caddy;then
  ...
  echo "Caddy 成功移动到 /usr/bin"
fi

# 创建并配置 Caddyfile
echo "正在创建和配置 Caddyfile"
...
if [[ -d "$cert_dir" ]];then
  if ls "$cert_dir"/* > /dev/null 2>&1;then
    echo "Caddy 证书申请成功，证书文件位于 $cert_dir"
  else
    echo "证书申请失败或仍在进行中。请检查 Caddy 日志获取更多信息。"
    exit 1
  fi
else
  echo "未找到与域名相关的证书目录：$cert_dir"
  exit 1
fi

# 输出 NaïveProxy 配置
echo "NaïveProxy.json 配置"
cat <<EOF
{
